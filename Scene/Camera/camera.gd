...
func calculate_target_position(room_center: Vector2, room_size: Vector2) -> Vector2:
	...
	# Clamps the return position to the left and right limits if the x_margin is positive
		...
		return_position.x = clamp(stage.platforming_player.position.x, left_limit, right_limit)
	...
	if y_margin <= 0:
		...
		return_position.y = clamp(stage.platforming_player.position.y, top_limit, bottom_limit)
